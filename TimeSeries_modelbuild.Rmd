...
tset <- seq(from=0, to=200, length.out = 5000) # definitely going to have to tweak time scale later
...
# set up empty vectors and initial values
A.simu <- NaN*tset; A.simu[1] <- 0
FI.simu <- NaN*tset; FI.simu[1] <- 0 # no parasites yet so this is 0
FU.simu <- NaN*tset; FU.simu[1] <- 0
SI.simu <- NaN*tset; SI.simu[1] <- 0 # no parasites yet so this is 0
...
for(i in 2:length(tset)){
	...
	# uninfected 2IH
	dS_U <- ((r_SU*S_U)                                              # births
	         - (D_U * e_DU * alpha_DUSU + D_I*e_DI*S_U*alpha_DISU)   # consumption by uninfected and infected predators 
...
plot(tset, A.simu,type='l',las=1,lwd=2,ylab='Population Size',xlab='Time', col=Acol,ylim=c(-100,100), xlim = c(0, 110))
lines(tset, FU.simu, type = 'l', las = 1, lwd = 2, col = Fcol)
lines(tset, FI.simu, type = 'l', las = 1, lty = 3, col = Fcol)
lines(tset, SU.simu, type = 'l', las = 1, lwd = 2, col = Scol)
lines(tset, SI.simu, type = 'l', las = 1, lty = 3, col = Scol)
...
legend(x=90,y=70,legend=c('Algae', '1st Intermediate Host','2nd Intermediate Host','Definitive Host'),lwd=2,col=c(Acol, Fcol,Scol,Dcol))
```
driving pop negative and then causing insane cycles, equilibria at zero
