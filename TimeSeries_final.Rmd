...
for(i in 2:length(tset)){
	...
	dS_U <- ((r_SU*S_U)                                                # births
	         - (D_U + D_I) * (gamma_D * ((S_U)/(h_D + (S_U))))         # type II consumption by uninfected and infected predators 
	...
	dD_U <- ((D_U + D_I) * (gamma_D * e_DU * ((v + S_U + alpha* S_I)/(h_D + (v + S_U + alpha * S_I)))) # births due to consumption of infected and uninfected prey, type II functional response
	         ...
	         - ((gamma_D*alpha*S_I)/(h_D + (v+S_U + alpha * S_I))))*dt                               # minus movement to infected class
	
	# infected DH
	dD_I <- ((D_U *((gamma_D * alpha * S_I)/(h_D + (v + S_U + alpha * S_I)))) # new infections from eating infected 2IH
	         - (D_I*d_DI))* dt                                        # minus death 
...
plot(tset, A.simu,type='l',las=1,lwd=2,ylab='Population Size',xlab='Time', col=Acol,ylim=c(0,500), xlim = c(0, 10))
